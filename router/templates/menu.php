...
<div class="menu container rounded-bottom-4 d-lg-flex d-sm-none d-none">
    ...
    if ((new Member())->setArray($Request->SESSION('member', true)->Get())->isInitialized()) {
        ?>
        <?= rand_commit($Config) ?>
...
<div class="menu2-panel d-lg-none d-sm-flex position-fixed vstack rounded-end-4" style="visibility: hidden">
    ...
    <?= rand_commit($Config) ?>
    <a class="logo" href="<?= $Utils->getInstanceAddress(true) ?>">
        <img class="rounded-circle" src="https://wadventure.blaetoan.cyou/img/favicon.ico" width="48px" height="48px"
             alt="LOGO 圖片">
    </a>
    <?= rand_commit($Config) ?>
    <a class="item mt-2 d-flex rounded-pill btn btn-primary" href="<?= $Utils->getInstanceAddress(true) ?>">
        <div class="icon">
    ...
    <?= rand_commit($Config) ?>
    <a class="item mt-2 d-flex rounded-pill btn btn-primary" href="<?= $Utils->getInstanceAddress(true) ?>/player.php">
        <div class="icon">
    ...
    <?= rand_commit($Config) ?>
    <a class="item mt-2 d-flex rounded-pill btn btn-primary" href="<?= $Utils->getInstanceAddress(true) ?>/online.php">
        <div class="icon">
    ...
    <?= rand_commit($Config) ?>
    <a class="item mt-2 d-flex rounded-pill btn btn-primary" href="<?= $Utils->getInstanceAddress(true) ?>/rank.php">
        <div class="icon">
    ...
    <?= rand_commit($Config) ?>
    <a class="item mt-2 d-flex rounded-pill btn btn-primary" href="<?= $Utils->getInstanceAddress(true) ?>/shop.php">
        <div class="icon">
    ...
    if ((new Member())->setArray($Request->SESSION('member', true)->Get())->isInitialized()) {
        ...
        <a class="item mt-2 d-flex rounded-pill btn btn-primary"
           href="<?= $Utils->getInstanceAddress(true) ?>/profile.php">
            <div class="icon">
        ...
        <?= rand_commit($Config) ?>
        <a class="item mt-2 d-flex rounded-pill btn btn-primary"
           href="<?= $Utils->getInstanceAddress(true) ?>/logout.php">
            <div class="icon">
    ...
    <?php } else { ?>
        ...
        <?= rand_commit($Config) ?>
        <a class="item mt-2 d-flex rounded-pill btn btn-primary"
           href="<?= $Utils->getInstanceAddress(true) ?>/login.php">
            <div class="icon">
